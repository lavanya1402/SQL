...
-- Selecting the state and counting the number of customers in each state
SELECT state, COUNT(customer_id) AS customer_count
FROM customer_t
-- Grouping the results by state
GROUP BY state
-- Ordering the results by customer count in descending order
...
      Now average the feedback for each quarter. */
  ...
  -- Creating a temporary table (CTE) named RatingValues
...
-- Selecting the quarter_number and calculating the average numeric rating
SELECT
    quarter_number,
    AVG(numeric_rating) AS average_rating
FROM
    RatingValues
-- Grouping the results by quarter_number
GROUP BY
    quarter_number
-- Ordering the results by quarter_number in descending order
ORDER BY
    quarter_number DESC;
...
Hint: Need the percentage of different types of customer feedback in each quarter. Use a common table expression and
   ...
   */   
   ...
   -- Creating a temporary table (CTE) named FeedbackCounts
WITH FeedbackCounts AS (
  SELECT
    quarter_number,
    COUNT(*) AS TotalFeedback,
    SUM(CASE WHEN customer_feedback = 'Very Bad' THEN 1 ELSE 0 END) AS VeryBadCount,
    SUM(CASE WHEN customer_feedback = 'Bad' THEN 1 ELSE 0 END) AS BadCount,
    SUM(CASE WHEN customer_feedback = 'Okay' THEN 1 ELSE 0 END) AS OkayCount,
    SUM(CASE WHEN customer_feedback = 'Good' THEN 1 ELSE 0 END) AS GoodCount,
    SUM(CASE WHEN customer_feedback = 'Very Good' THEN 1 ELSE 0 END) AS VeryGoodCount
  FROM order_t
  GROUP BY quarter_number
)

-- Selecting the quarter_number and calculating the percentage split
SELECT
  quarter_number,
  TotalFeedback,
  (VeryBadCount / TotalFeedback) * 100 AS PercentageVeryBad,
  (BadCount / TotalFeedback) * 100 AS PercentageBad,
  (OkayCount / TotalFeedback) * 100 AS PercentageOkay,
  (GoodCount / TotalFeedback) * 100 AS PercentageGood,
  (VeryGoodCount / TotalFeedback) * 100 AS PercentageVeryGood
FROM FeedbackCounts;
...
-- Selecting the vehicle_maker and counting the orders for each maker
SELECT
    p.vehicle_maker,
...
-- Creating a ranked list of vehicle makers for each state based on distinct customer count
WITH RankedVehicleMakes AS (
  SELECT
    c.state,
    p.vehicle_maker,
    COUNT(DISTINCT c.customer_id) AS CustomerCount,
    RANK() OVER (PARTITION BY c.state ORDER BY COUNT(DISTINCT c.customer_id) DESC) AS rank_num
  FROM
    customer_t c
  JOIN
...
-- Selecting the most preferred vehicle maker for each state
SELECT
  state,
  vehicle_maker,
  CustomerCount,
...
Hint: Count the number of orders for each quarter.*/
    ...
    -- Counting the number of orders for each quarter
SELECT
    quarter_number,
...
*/
    ...
    SELECT
        o.product_id,
        o.quarter_number,
        SUM(p.vehicle_price * o.quantity) AS total_income
    FROM
        order_t o
    JOIN
        product_t p ON o.product_id = p.product_id
    GROUP BY
        o.product_id, o.quarter_number
)

SELECT
    quarter_number,
    total_income_for_quarter,
    LAG(total_income_for_quarter) OVER (ORDER BY quarter_number) AS prev_quarter_income,
    (total_income_for_quarter - LAG(total_income_for_quarter) OVER (ORDER BY quarter_number)) / LAG(total_income_for_quarter) OVER (ORDER BY quarter_number) * 100 AS qoq_percentage_change
FROM (
    SELECT
        quarter_number,
        SUM(total_income) AS total_income_for_quarter
    FROM
        RevenueCTE
    GROUP BY
        quarter_number
) AS QuarterIncomeSummary
ORDER BY
    quarter_number;
...
WITH RevenueAndOrdersCTE AS (
    SELECT
        o.quarter_number,
        COUNT(o.order_id) AS order_count,
        SUM(o.quantity * p.vehicle_price) AS total_revenue
    FROM
        order_t o
    JOIN
        product_t p ON o.product_id = p.product_id
